...
code = code.replace(/        for \(const projectId of client.projectIds \|\| \[\]\)[\s\n]*if \(\!projectIds.has\(projectId\)\) throw new Error\(\`OAuth client override '\$\{mapping.username\}\/\$\{clientId\}' references an unknown project\`\);/, 
\`        if ((client.projectIds || []).some(id => !projectIds.has(id))) throw new Error(\\\`OAuth client override '\${mapping.username}/\${clientId}' references an unknown project\\\`);\`);
