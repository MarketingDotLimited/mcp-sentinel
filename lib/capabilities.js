...
export function isLegacyTool(name) {
...
export async function toolAvailability(name) {
  if (LEGACY_TOOLS.has(name) && process.env.ENABLE_LEGACY_TOOLS !== 'true')
    return {
      available: false,
      deprecated: true,
      pack: 'legacy-compatibility',
      message: 'This legacy capability is disabled. Export legacy state before the 2.0 removal migration.',
    };
  if (LEGACY_TOOLS.has(name))
    return {
      available: true,
      deprecated: true,
      message: 'This legacy capability is deprecated and will be removed after the next minor release.',
    };
