...
export class JobQueue {
  ...
  complete(id, result = {}, { workerId } = {}) {
    assertText(workerId, 'Worker ID', 256);
    return this.#finish(id, 'completed', result, null, workerId);
  }

  fail(id, error, { retry = true, workerId } = {}) {
    assertText(workerId, 'Worker ID', 256);
    const row = this.get(id);
    if (!row) throw new Error('Job not found');
    const shouldRetry = retry && row.attempts < row.maxAttempts;
    const state = shouldRetry ? 'queued' : 'failed';
    return this.#finish(id, state, null, String(error?.message || error || 'Job failed'), workerId);
  }
  ...
  startWorker({ workerId, handlers = {}, pollMs = 250, leaseMs = 60000 } = {}) {
    ...
    const run = async () => {
      ...
      if (typeof handler !== 'function') {
        this.fail(job.id, `No registered handler for job type '${job.type}'`, { retry: false, workerId });
      } else {
        try {
          this.complete(job.id, await handler(job.payload, job), { workerId });
        } catch (error) {
          this.fail(job.id, error, { workerId });
        }
      }
  ...
  #finish(id, state, result, error, workerId = null) {
    ...
    const now = new Date().toISOString();
    if (state !== 'cancelled') assertText(workerId, 'Worker ID', 256);
    const outcome = this.database
      .prepare(
        `UPDATE sentinel_jobs SET state=?, result=?, error=?, lease_until=NULL, worker_id=NULL, updated_at=?
         WHERE id=? AND state IN (?,?)${workerId ? ' AND worker_id=?' : ''}`
      )
      .run(
        state,
