...
async function loadPolicy() {
  ...
  try {
    ...
    if (!parsed || !Array.isArray(parsed.rules) || !parsed.rules.every(rule => ['deny', 'require_approval'].includes(rule.effect) && Array.isArray(rule.tools))) {
...
export async function evaluatePolicy({ tool, identity }) {
  ...
  if (matched.some(rule => rule.effect === 'deny')) return { allowed: false, requireApproval: false, reason: 'This action is denied by server policy' };
