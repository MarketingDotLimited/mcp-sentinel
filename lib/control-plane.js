...
const FLOW_CONTROL_KEYS = new Set(['flowId', 'resumeFromPassed', 'forceReplay']);
...
function actionHash({ tool, args, identity, sshPolicyVersion = 0, flowId = null }) {
  return crypto
    .createHash('sha256')
    .update(
      JSON.stringify(
...
export async function requestApproval({ tool, args, identity, summary, risk = 'high' }) {
  ...
  const hashes = matchingActionHashes({ tool, args, identity, state, flowId });
