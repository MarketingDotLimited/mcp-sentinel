...
const FLOW_CONTROL_KEYS = new Set(['flowId', 'resumeFromPassed', 'forceReplay', 'flowStep']);
...
function normalizeActionArgsForHash(args = {}) {
  if (!args || typeof args !== 'object' || Array.isArray(args)) return args;
  return Object.fromEntries(Object.entries(args).filter(([key]) => !FLOW_CONTROL_KEYS.has(key)));
}

function actionHash({ tool, args, identity, sshPolicyVersion = 0, flowId = null, flowStep = null }) {
  return crypto
    .createHash('sha256')
    .update(
      JSON.stringify(
        canonicalize({
          tool,
          args: normalizeActionArgsForHash(args),
          flowId: flowId || null,
...
function matchingActionHashes({ tool, args, identity, state, flowId }) {
  const flowStep = typeof args?.flowStep === 'string' ? args.flowStep.trim() : '';
  const canonical = {
    tool,
    args,
    identity,
  ...
  const hashes = [];
  hashes.push(actionHash(canonical));
  if (flowId !== null) {
    hashes.push(
      actionHash({
        ...canonical,
...
function safeHttpUrl(value, envName) {
  ...
  } catch { throw new Error('A valid HTTP or HTTPS URL is required'); }
...
export async function requestApproval({ tool, args, identity, summary, risk = 'high' }) {
  ...
  const flowId = typeof args?.flowId === 'string' ? args.flowId.trim() || null : null;
  const flowStep = typeof args?.flowStep === 'string' ? args.flowStep.trim() || null : null;
  const hashes = matchingActionHashes({ tool, args: { ...args, flowStep }, identity, state, flowId });
  const existing = state.approvals.find(
    item => hashes.includes(item.actionHash) && item.status === 'pending' && !isExpired(item)
  );
  if (existing) return { approval: existing, created: false };
  const previousFailure = state.approvals
    .filter(item => hashes.includes(item.actionHash) && item.status === 'failed')
    .sort((a, b) => String(b.failedAt || '').localeCompare(String(a.failedAt || '')))[0];
  ...
  const approval = {
    ...
    actionHash: hashes[0],
    retryOf: previousFailure?.id || null,
    attempt: (Number(previousFailure?.attempt) || 0) + 1,
    flowId,
    decisionHistory: [
      {
        state: previousFailure ? 'retry-pending' : 'pending',
        at: now.toISOString(),
        by: identity.userId,
        ...(previousFailure ? { retryOf: previousFailure.id } : {}),
      },
    ],
  };
...
export async function consumeApproval({ tool, args, identity }) {
  ...
  const approval = state.approvals.find(
    item => hashes.includes(item.actionHash) && item.status === 'approved' && !isExpired(item)
  );
  if (!approval) return null;
  approval.status = 'executing';
  approval.executionStartedAt = new Date().toISOString();
  approval.executionGrantId = crypto.randomUUID();
  approval.decisionHistory ||= [];
  approval.decisionHistory.push({
    state: 'executing',
    at: approval.executionStartedAt,
    by: identity.userId,
    grantId: approval.executionGrantId,
  });
  await saveState(state);
  return approval;
}

export async function getLatestSuccessfulExecution({ tool, args, identity }) {
  const state = await loadState();
  const flowId = typeof args?.flowId === 'string' && args.flowId.trim() ? args.flowId.trim() : null;
  const flowStep = typeof args?.flowStep === 'string' ? args.flowStep.trim() || null : null;
  const hashes = matchingActionHashes({ tool, args: { ...args, flowStep }, identity, state, flowId });
  const candidates = state.approvals.filter(item => {
    return hashes.includes(item.actionHash) && item.status === 'executed' && !isExpired(item);
  });
  if (!candidates.length) return null;
  const sorted = [...candidates].sort((a, b) => String(b.executedAt || '').localeCompare(String(a.executedAt || '')));
  return sorted[0];
}

function executionApproval(state, id, identity) {
  const approval = state.approvals.find(item => item.id === id);
  if (!approval || approval.status !== 'executing') throw new Error('Approval execution grant is not active');
  const expected = approval.requestedBy;
  if (
    expected.userId !== identity.userId ||
    (expected.oauthSubject || null) !== (identity.oauthSubject || null) ||
    (expected.oauthClient || null) !== (identity.oauthClient || null)
  )
    throw new Error('Approval execution identity does not match the requester');
  return approval;
}

export async function completeApprovalExecution(id, identity) {
  const state = await loadState();
  const approval = executionApproval(state, id, identity);
  ...
  await saveState(state);
  return { id: approval.id, status: approval.status, executedAt: approval.executedAt };
}

export async function failApprovalExecution(id, identity, reason) {
  const state = await loadState();
  const approval = executionApproval(state, id, identity);
  approval.status = 'failed';
  approval.failedAt = new Date().toISOString();
  approval.failureReason = String(reason || 'Operation failed')
    .replace(/[\r\n\t]+/g, ' ')
    .slice(0, 500);
  approval.decisionHistory ||= [];
  approval.decisionHistory.push({
    state: 'failed',
    at: approval.failedAt,
    by: identity.userId,
    reason: approval.failureReason,
  });
  await saveState(state);
  return { id: approval.id, status: approval.status, failedAt: approval.failedAt };
}
...
function parseTextField(value, label, opts = {}) {
  if (typeof value !== 'string') {
    if (opts.optional) return '';
    throw new Error(`${label} is required`);
  }
  const normalized = value.trim();
  if (!normalized && !opts.optional) throw new Error(`${label} is required`);
  if (normalized.length > (opts.max || 255)) throw new Error(`${label} is too long`);
  return normalized;
}

function identityPreferenceTargetByType(input, targetType) {
  if (targetType === 'identity-key') {
    const keyId = parseTextField(input.keyId, 'keyId');
    return {
      id: identitySshPreferenceId({ keyId }),
      base: {
        identityType: 'apiKey',
        keyId,
        userId: null,
  ...
  const hasKeyId = typeof input.keyId === 'string' && input.keyId.trim();
  const hasUserId = typeof input.userId === 'string' && input.userId.trim();
  const hasIssuer = typeof input.issuer === 'string' && input.issuer.trim();
  const hasSubject = typeof input.subject === 'string' && input.subject.trim();
  const hasAuth = typeof input.authType === 'string' && input.authType.trim();
  const selectedAuth = hasAuth ? input.authType.trim() : null;

  if (!selectedAuth) {
    const selectedCount = [!!hasIssuer && !!hasSubject, !!hasKeyId, !!hasUserId].filter(Boolean).length;
    if (selectedCount === 0) throw new Error('identity target requires userId, keyId, or oauth issuer+subject');
    if (selectedCount > 1) throw new Error('identity target selectors are ambiguous; set authType explicitly');
  }

  const authType = selectedAuth || (hasIssuer && hasSubject ? 'oauth' : hasKeyId ? 'apiKey' : 'local');

  if (authType === 'oauth') {
    const issuer = parseTextField(input.issuer, 'issuer');
    const subject = parseTextField(input.subject, 'subject', { max: 512 });
    return {
      id: identitySshPreferenceId({ authType: 'oauth', oauthIssuer: issuer, oauthSubject: subject }),
      base: {
        identityType: 'oauth',
        keyId: null,
        userId: null,
  ...
  if (authType === 'local') {
    const userId = parseTextField(input.userId, 'userId');
    return {
      id: identitySshPreferenceId({ userId }),
      base: {
        identityType: 'local',
        keyId: null,
        userId,
...
function recordPolicyChange(state, { targetType, targetId, before, after, identity }) {
  ...
  const global = state.sshPolicies.find(item => item.id === 'global');
...
function identityPreferenceTarget(input) {
  return identityPreferenceTargetByType(input, 'identity');
}
...
export async function adminSetSshAccess(input, identity) {
  ...
  } else if (type === 'identity' || type === 'identity-key') {
    const target = parseTextField(input.targetId || '', 'targetId', { optional: true });
    /* c8 ignore next 3 */
    if (type === 'identity' && target && /[^a-zA-Z0-9_.-]/.test(target)) {
      throw new Error('identity targetId is not supported');
    }
    const identityTarget = identityPreferenceTargetByType(input, type);
    records = state.identitySshPreferences;
    targetId = identityTarget.id;
    base = identityTarget.base;
  } else if (type === 'oauth-client') {
    const issuer = parseTextField(input.issuer, 'issuer');
    const clientId = parseTextField(input.clientId, 'clientId', { max: 255 });
    targetId = oauthClientSshPolicyId(issuer, clientId);
    records = state.oauthClientSshPolicies;
    base = { ...base, issuer, clientId };
  } else if (type === 'subject-client') {
    const targetIdentity = {
      oauthIssuer: parseTextField(input.issuer, 'issuer'),
      oauthSubject: parseTextField(input.subject, 'subject', { max: 512 }),
      oauthClient: parseTextField(input.clientId, 'clientId', { max: 255 }),
    };
    targetId = subjectClientSshPreferenceId(targetIdentity);
    records = state.subjectClientSshPreferences;
    base = {
      issuer: targetIdentity.oauthIssuer,
      subject: targetIdentity.oauthSubject,
      clientId: targetIdentity.oauthClient,
    };
  } else {
...
function validateProject(input) {
  ...
  const testNetworkHosts = Array.isArray(input.testNetworkHosts)
    ? [...new Set(input.testNetworkHosts.map(value => String(value).trim().toLowerCase()))]
    : [];
  if (testNetworkHosts.length > 20 || testNetworkHosts.some(value => isIP(value) === 0))
    throw new Error('testNetworkHosts must contain at most 20 explicit IP addresses');
  return {
    name,
    rootPath,
    repoPath,
