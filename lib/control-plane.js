...
function encryptionKey(keyId = process.env.CONTROL_PLANE_KEY_ID || 'state-key-v1') {
  const currentKeyId = process.env.CONTROL_PLANE_KEY_ID || 'state-key-v1';
  let value = keyId === currentKeyId ? process.env.CONTROL_PLANE_ENCRYPTION_KEY || '' : '';
  if (!value && process.env.CREDENTIALS_DIRECTORY) {
    try {
      const credentialName = keyId === currentKeyId ? 'state-key' : `state-key-${keyId}`;
      if (!/^[A-Za-z0-9_.-]{1,80}$/.test(credentialName)) throw new Error('Invalid encryption key ID');
      value = fsSync.readFileSync(path.join(process.env.CREDENTIALS_DIRECTORY, credentialName), 'utf8').trim();
    } catch {}
  }
...
function encryptSecret(value) {
  ...
  return {
    v: 1,
    keyId: process.env.CONTROL_PLANE_KEY_ID || 'state-key-v1',
...
function decryptSecret(value) {
  const decipher = crypto.createDecipheriv(
    'aes-256-gcm',
    encryptionKey(value.keyId || process.env.CONTROL_PLANE_KEY_ID || 'state-key-v1'),
...
export async function getProjectByExactPath(projectPath, identity) {
  const requested = path.resolve(String(projectPath || ''));
  const projects = await listProjects(identity);
  const project = projects.find(item => path.resolve(item.rootPath || item.repoPath) === requested);
  if (!project) throw new Error('Legacy projectPath does not exactly match an assigned registered project');
  return project;
}
...
export async function exportLegacyState(identity) {
  if (identity.role !== 'admin') throw new Error('Only administrators can export legacy state');
  const state = await loadState();
  return {
    exportedAt: new Date().toISOString(),
    schemaVersion: state.version,
    automations: state.automations.map(item => redact(item)),
    fleet: state.fleets.map(item => redact(item)),
    backupTargets: state.backupTargets.map(item => redact(item)),
    webhooks: state.webhooks.map(item => redact(item)),
  };
}
