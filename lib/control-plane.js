...
const WORKFLOWS = [
  ...
  },
  {
    ...
    description: 'Guides an AI through repository setup, tests, deployment, health checks, and rollback planning.',
    risk: 'approval-required',
    prompts: ['Inspect the project', 'Run tests', 'Prepare a deployment plan', 'Ask for approval before deployment'],
  },
  {
    id: 'create-backup',
    title: 'Protect my configuration',
    description: 'Creates a safe configuration-change plan with validation and rollback.',
    risk: 'approval-required',
    prompts: ['Identify the configuration', 'Create a rollback plan', 'Validate before applying changes'],
  },
];
  ...
  if (!statePromise) {
    statePromise = (async () => {
      try {
        ...
        return { version: 1, projects: [], automations: [], organizations: [], teams: [], fleets: [], backupTargets: [], webhooks: [], ...parsed };
      } catch (err) {
        if (err.code === 'ENOENT') return { version: 1, approvals: [], projects: [], automations: [], organizations: [], teams: [], fleets: [], backupTargets: [], webhooks: [] };
        throw err;
      }
    })();
  }
...
function encryptionKey() {
  ...
  if (!/^[a-f0-9]{64}$/i.test(value)) throw new Error('CONTROL_PLANE_ENCRYPTION_KEY must be a 64-character hexadecimal key before storing secrets');
...
function allowedHost(url, envName) {
  const hosts = (process.env[envName] || '').split(',').map(value => value.trim().toLowerCase()).filter(Boolean);
...
function safeHttpUrl(value, envName) {
  let url;
  try { url = new URL(String(value || '')); } catch { throw new Error('A valid HTTP or HTTPS URL is required'); }
  if (!['http:', 'https:'].includes(url.protocol) || url.username || url.password) throw new Error('URL must use HTTP or HTTPS and cannot include credentials');
...
function allowedPath(value, envName) {
  ...
  const roots = (process.env[envName] || '').split(',').map(item => item.trim()).filter(Boolean).map(item => path.resolve(item));
  if (!roots.length) throw new Error(`${envName} must list allowed absolute paths`);
  if (!roots.some(root => resolved === root || resolved.startsWith(`${root}${path.sep}`))) throw new Error(`Path must be inside a directory listed in ${envName}`);
  return resolved;
}
...
async function fetchWithTimeout(url, options = {}, timeoutMs = 10_000) {
  ...
  try { return await fetch(url, { ...options, redirect: 'error', signal: controller.signal }); }
...
export async function listApprovals(identity, { includeResolved = false } = {}) {
  ...
  const visible = identity.role === 'admin' ? approvals : approvals.filter(item => item.requestedBy.userId === identity.userId);
  return visible
...
export async function requestApproval({ tool, args, identity, summary, risk = 'high' }) {
  ...
  const existing = state.approvals.find(item => item.actionHash === hash && item.status === 'pending' && !isExpired(item));
...
export async function consumeApproval({ tool, args, identity }) {
  const state = await loadState();
  const hash = actionHash({ tool, args, identity });
  const approval = state.approvals.find(item => item.actionHash === hash && item.status === 'approved' && !isExpired(item));
...
function validateProject(input) {
  ...
  if (!/^[a-zA-Z0-9][a-zA-Z0-9 _.-]{1,79}$/.test(name)) throw new Error('Project name must be 2-80 letters, numbers, spaces, dots, underscores, or hyphens');
  const repoPath = String(input.repoPath || '').trim();
  const allowedRepos = (process.env.GIT_ALLOWED_REPOS || '').split(',').map(item => item.trim()).filter(Boolean);
  ...
  if (healthUrl) {
    ...
    } catch { throw new Error('healthUrl must be a valid HTTP or HTTPS URL'); }
  }
  ...
  return { name, repoPath, serviceName, healthUrl, environment, description: String(input.description || '').trim().slice(0, 500) };
}
...
export async function createProject(input, identity) {
  ...
  const state = await loadState();
  const project = { id: crypto.randomUUID(), ...validateProject(input), createdAt: new Date().toISOString(), createdBy: identity.userId };
  if (state.projects.some(item => item.name.toLowerCase() === project.name.toLowerCase())) throw new Error('A project with this name already exists');
...
export function getDeploymentPlan(project) {
  const steps = [
    { stage: 'Inspect', action: `Review the working tree and recent changes in ${project.repoPath}.`, tool: 'git_operation' },
    { stage: 'Validate', action: 'Run the project test and build commands in a sandbox or approved CI environment.', tool: 'run_sandboxed_code' },
    { stage: 'Deploy', action: project.serviceName ? `Deploy the approved revision and restart ${project.serviceName}.` : 'Deploy the approved revision using the project deployment process.', tool: 'manage_service' },
    { stage: 'Verify', action: project.healthUrl ? `Check ${project.healthUrl} and service health after deployment.` : 'Check service health and recent logs after deployment.', tool: 'get_service_status' },
    { stage: 'Recover', action: 'If validation fails, restore the previous configuration or revision and report the outcome.', tool: 'restore_config' },
  ];
...
export async function createAutomation(input, identity) {
  ...
  if (!['health_check', 'backup'].includes(input.type)) throw new Error('Only health_check and backup automations are supported');
  const intervalMinutes = Number(input.intervalMinutes || 60);
  if (!Number.isInteger(intervalMinutes) || intervalMinutes < 5 || intervalMinutes > 10080) throw new Error('intervalMinutes must be between 5 and 10080');
  ...
  const automation = {
    id: crypto.randomUUID(), name, type: input.type, intervalMinutes, enabled: input.enabled !== false,
    ...
    createdAt: new Date().toISOString(), createdBy: identity.userId, nextRunAt: new Date().toISOString(), lastRunAt: null, lastResult: null,
  };
...
export async function registerFleetServer(input, identity) {
  ...
  const state = await loadState();
  if (state.fleets.some(item => item.name.toLowerCase() === name.toLowerCase() || item.healthUrl === healthUrl)) throw new Error('A server with that name or health URL is already registered');
  const server = { id: crypto.randomUUID(), name, healthUrl, labels: Array.isArray(input.labels) ? input.labels.filter(item => typeof item === 'string' && /^[a-zA-Z0-9_.-]{1,40}$/.test(item)).slice(0, 20) : [], createdAt: new Date().toISOString(), createdBy: identity.userId, lastCheck: null };
...
export async function checkFleetServer(id, identity) {
  ...
  try {
    ...
    server.lastCheck = { status: response.ok ? 'healthy' : 'unhealthy', checkedAt: new Date().toISOString(), httpStatus: response.status, latencyMs: Date.now() - started };
  } catch (err) {
    server.lastCheck = { status: 'unreachable', checkedAt: new Date().toISOString(), message: err.name === 'AbortError' ? 'Timed out' : 'Connection failed', latencyMs: Date.now() - started };
  }
  await saveState(state);
...
export async function createBackupTarget(input, identity) {
  ...
  const state = await loadState();
  const target = { id: crypto.randomUUID(), name, type: input.type, createdAt: new Date().toISOString(), createdBy: identity.userId };
  if (input.type === 'local') target.destination = allowedPath(input.destination, 'BACKUP_ALLOWED_ROOTS');
    ...
    target.endpoint = endpoint; target.bucket = bucket; target.region = String(input.region || 'us-east-1').slice(0, 64);
    target.credentials = encryptSecret({ accessKeyId, secretAccessKey });
  }
  if (state.backupTargets.some(item => item.name.toLowerCase() === name.toLowerCase())) throw new Error('A backup target with this name already exists');
...
export async function runBackup({ targetId, sourcePath }) {
  ...
  if (!sourceStat.isFile()) throw new Error('Only regular files can be backed up; use a dedicated archive process for directories');
  ...
  const payload = encryptSecret({ source: path.basename(source), createdAt: new Date().toISOString(), data: plaintext.toString('base64') });
  ...
  await client.send(new PutObjectCommand({ Bucket: target.bucket, Key: `mcp-sentinel/${filename}`, Body: JSON.stringify(payload), ContentType: 'application/json' }));
  return { id, targetId, filename, bytes: sourceStat.size, destination: `s3://${target.bucket}/mcp-sentinel/${filename}`, encrypted: 'AES-256-GCM' };
}
...
export async function createWebhook(input, identity) {
  ...
  const state = await loadState();
  if (state.webhooks.some(item => item.name.toLowerCase() === name.toLowerCase())) throw new Error('A webhook with this name already exists');
  const webhook = { id: crypto.randomUUID(), name, url, events: Array.isArray(input.events) ? input.events.filter(event => /^[a-z0-9._-]{1,80}$/.test(event)).slice(0, 30) : ['deployment.completed'], secret: encryptSecret(secret), createdAt: new Date().toISOString(), createdBy: identity.userId, lastDelivery: null };
  if (!webhook.events.length) throw new Error('At least one valid event is required');
  state.webhooks.push(webhook); await saveState(state);
  const { secret: _secret, ...safe } = webhook; return safe;
}
...
export async function deliverWebhook({ webhookId, event, payload }, identity) {
  ...
  const state = await loadState(); const webhook = state.webhooks.find(item => item.id === webhookId);
  if (!webhook) throw new Error('Webhook not found');
  if (!webhook.events.includes(event)) throw new Error('This webhook is not subscribed to that event');
  const body = JSON.stringify({ id: crypto.randomUUID(), event, sentAt: new Date().toISOString(), data: payload || {} });
  const signature = crypto.createHmac('sha256', decryptSecret(webhook.secret)).update(body).digest('hex');
  const response = await fetchWithTimeout(webhook.url, { method: 'POST', headers: { 'content-type': 'application/json', 'user-agent': 'MCP-Sentinel/1.0', 'x-mcp-sentinel-signature-256': `sha256=${signature}` }, body });
  webhook.lastDelivery = { event, deliveredAt: new Date().toISOString(), httpStatus: response.status, status: response.ok ? 'delivered' : 'failed' };
  await saveState(state);
...
export async function listOrganizations(identity) {
  ...
  const organizations = identity.organizationId ? state.organizations.filter(item => item.id === identity.organizationId) : [];
...
export async function createOrganization({ name }, identity) {
  ...
  const state = await loadState();
  const organization = { id: crypto.randomUUID(), name: validateNamedEntity(name, 'Organization'), createdAt: new Date().toISOString(), createdBy: identity.userId };
  if (state.organizations.some(item => item.name.toLowerCase() === organization.name.toLowerCase())) throw new Error('An organization with this name already exists');
...
export async function createTeam({ name, organizationId, projectIds = [], role = 'developer' }, identity) {
  ...
  if (!['viewer', 'auditor', 'developer', 'operator'].includes(role)) throw new Error('Team role must be viewer, auditor, developer, or operator');
  if (!Array.isArray(projectIds) || !projectIds.every(id => typeof id === 'string' && state.projects.some(project => project.id === id))) throw new Error('Each team project must be registered');
  const team = { id: crypto.randomUUID(), name: validateNamedEntity(name, 'Team'), organizationId, role, projectIds, createdAt: new Date().toISOString(), createdBy: identity.userId };
...
export async function validateKeyAssignment({ organizationId, teamId }) {
  const state = await loadState();
  if (organizationId && !state.organizations.some(item => item.id === organizationId)) throw new Error('Organization not found');
  if (!teamId) return;
  const team = state.teams.find(item => item.id === teamId);
  if (!team) throw new Error('Team not found');
  if (organizationId && team.organizationId !== organizationId) throw new Error('Team does not belong to the selected organization');
}
