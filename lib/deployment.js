...
export function validateProjectWritePaths(value) {
  const paths = [...new Set(String(value || '').split(',').map(item => item.trim()).filter(Boolean))];
  if (!paths.length) throw new Error('BROKER_GIT_ALLOWED_REPOS must contain at least one project repository');
  for (const item of paths) {
    if (/[\0\r\n\t "'\\]/.test(item) || !path.isAbsolute(item) || path.resolve(item) !== item)
      throw new Error(`Project write path '${item}' must be a normalized absolute path without whitespace`);
    if (['/', '/etc', '/usr', '/var', '/var/www', '/srv', '/opt', '/root', '/home'].includes(item))
      throw new Error(`Project write path '${item}' is too broad`);
  }
  return paths;
}
