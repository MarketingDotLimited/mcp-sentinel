...
export async function openSqliteState(databaseFile, legacyFile) {
  ...
  const migration6 = database.prepare('SELECT version FROM schema_migrations WHERE version = 6').get();
  if (!migration6 && stateEncryptionConfigured()) {
    const now = new Date().toISOString();
    database.exec('BEGIN IMMEDIATE');
    try {
      for (const table of Object.values(DOMAINS)) {
        const update = database.prepare(`UPDATE ${table} SET payload = ?, updated_at = ? WHERE id = ?`);
        for (const row of database.prepare(`SELECT id, payload FROM ${table}`).all()) {
          const payload = JSON.parse(row.payload);
          update.run(JSON.stringify(encryptSensitiveFields(payload)), now, row.id);
        }
      }
      const updateRun = database.prepare('UPDATE task_runs SET payload = ?, updated_at = ? WHERE id = ?');
      for (const row of database.prepare('SELECT id, payload FROM task_runs').all()) {
        const payload = JSON.parse(row.payload);
        updateRun.run(
          JSON.stringify(isEncryptedStateValue(payload) ? payload : encryptStateValue(payload)),
          now,
          row.id
        );
      }
...
export function saveSqliteState(database, state) {
  ...
  try {
    for (const [key, table] of Object.entries(DOMAINS)) {
      ...
      for (const item of state[key] || []) insert.run(item.id, JSON.stringify(encryptSensitiveFields(item)), now);
    }
    database.exec('COMMIT');
  } catch (error) {
    database.exec('ROLLBACK');
...
export function upsertTaskRun(database, run) {
  ...
  database
    ...
    .run(
      ...
      run.owner,
      run.state,
      JSON.stringify(encryptStateValue(run)),
...
export function loadTaskRun(database, runId) {
  ...
  return row ? decryptStateValue(JSON.parse(row.payload)) : null;
}
