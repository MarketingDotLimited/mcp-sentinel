...
function normalizeAuthorizationMapping(input, existing = {}) {
  ...
  if (!Array.isArray(projectIds) || !projectIds.every(id => /^[0-9a-f-]{36}$/i.test(id)))
    throw new Error('Invalid OAuth project assignments');
  if (
    input.clients !== undefined &&
    (!input.clients || typeof input.clients !== 'object' || Array.isArray(input.clients))
  )
  ...
  return {
    linuxUser: input.linuxUser ?? existing.linuxUser ?? '',
    role,
    scopes: [...new Set(scopes)],
    requireApproval: input.requireApproval ?? existing.requireApproval ?? true,
    projectIds: [...new Set(projectIds)],
    organizationId: input.organizationId ?? existing.organizationId ?? null,
    teamId: input.teamId ?? existing.teamId ?? null,
    clients: input.clients ?? existing.clients ?? {},
...
export async function addOAuthUser({ username, password, email, groups, ...authorization }) {
  ...
  // Save mapping
  const mappings = await readMappings();
  mappings[username] = normalizeAuthorizationMapping(authorization);
...
export async function updateOAuthUser(username, updates) {
  ...
  if (
    ...
    const mappings = await readMappings();
    mappings[username] = normalizeAuthorizationMapping(updates, mappings[username] || {});
    await writeMappings(mappings);
  }
