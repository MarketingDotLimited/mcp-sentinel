...
export async function deleteOAuthClient(clientId) {
  ...
  // For deletion, we must remove the block from the text safely to preserve comments/formatting
  ...
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].match(/^\s+clients:\s*$/)) {
      inClients = true;
      continue;
    }
    if (inClients) {
      if (lines[i].match(/^\s+- client_id:\s+/) && clientStart !== -1 && clientEnd === -1) {
        clientEnd = i - 1;
      }
      if (lines[i].includes(`- client_id: ${clientId}`)) {
        clientStart = i;
      }
      if (lines[i].match(/^\s+[a-zA-Z]/) && !lines[i].match(/^\s+- /)) {
        inClients = false;
        if (clientStart !== -1 && clientEnd === -1) clientEnd = i - 1;
      }
    }
  }
  if (clientStart !== -1 && clientEnd === -1) clientEnd = lines.length - 1;

  if (clientStart !== -1) {
    lines.splice(clientStart, clientEnd - clientStart + 1);
  }
