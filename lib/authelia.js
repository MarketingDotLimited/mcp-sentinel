...
export async function addOAuthClient({ clientId, clientName, redirectUris }) {
  ...
  // We must preserve the JWKS private key block exactly as-is.
  // js-yaml will re-serialize multi-line strings differently,
  // so we do a targeted insertion instead of full re-dump for the config file.
  const newClient = yaml.dump([{
    client_id: clientId,
    client_name: clientName || clientId,
    public: true,
    authorization_policy: 'one_factor',
  ...
  // Append the new client block to the end of the config file
  const indentedClient = newClient
    .replace(/^- /m, '      - ')
    .replace(/^  /gm, '        ')
    .replace(/^        - client_id/, '      - client_id');

  const updatedRaw = raw.trimEnd() + '\n' + indentedClient + '\n';
  await safeWriteAndRestart(CONFIG_FILE, updatedRaw);
...
export async function deleteOAuthClient(clientId) {
  ...
  // For deletion we must do a full re-dump since we're removing from the middle.
  // This is riskier because js-yaml may reformat the private key block.
  // So we extract the key block first and re-inject it.
  const keyMatch = raw.match(/(      - key: \|[\s\S]*?-----END PRIVATE KEY-----)/);
  const keyBlock = keyMatch ? keyMatch[1] : null;

  const newYaml = yaml.dump(parsed, { lineWidth: -1, quotingType: '"', forceQuotes: false });

  // If the key block was mangled, restore it
  let finalYaml = newYaml;
  if (keyBlock) {
    // The yaml.dump will have changed the key formatting. We need to find and replace it.
    const keyStartMarker = '      - key:';
    const endKeyMarker = '-----END PRIVATE KEY-----';
    const start = finalYaml.indexOf(keyStartMarker);
    const end = finalYaml.indexOf(endKeyMarker);
    if (start !== -1 && end !== -1) {
      finalYaml = finalYaml.substring(0, start) + keyBlock + finalYaml.substring(end + endKeyMarker.length);
    }
  }

  await safeWriteAndRestart(CONFIG_FILE, finalYaml);
  return true;
}
...
export async function getAutheliaHealth() {
  try {
    ...
    return {
      ...
      url: 'https://begin.shopping:2083',
      discoveryUrl: 'https://begin.shopping:2083/.well-known/openid-configuration',
      jwksUrl: 'https://begin.shopping:2083/jwks.json',
    };
  } catch {
    return {
      status: 'inactive',
