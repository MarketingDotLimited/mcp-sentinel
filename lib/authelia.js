...
const TEMPLATE_PLACEHOLDER_PREFIX = '__MCP_SENTINEL_AUTHELIA_TEMPLATE_';

function parseAutheliaConfig(raw) {
  const templates = [];
  const parseable = raw.replace(
    /^(\s*-\s+key:\s*)(\{\{\s*secret\s+"[^"\n]+"\s*\|\s*mindent\s+\d+\s+"\|"\s*\|\s*msquote\s*\}\})\s*$/gm,
    (_line, prefix, template) => {
      const placeholder = `${TEMPLATE_PLACEHOLDER_PREFIX}${templates.length}__`;
      templates.push({ placeholder, template });
      return `${prefix}${placeholder}`;
    }
  );
  return { parsed: yaml.load(parseable), templates };
}

function dumpAutheliaConfig(parsed, templates) {
  let output = yaml.dump(parsed, { lineWidth: -1, noRefs: true });
  for (const { placeholder, template } of templates) {
    const escaped = placeholder.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const pattern = new RegExp(`^(\\s*-\\s+key:\\s*)['"]?${escaped}['"]?\\s*$`, 'm');
    if (!pattern.test(output)) throw new Error('Authelia signing-key template was not preserved');
    output = output.replace(pattern, `$1${template}`);
  }
  return output;
}
...
function normalizeAuthorizationMapping(input, existing = {}) {
  ...
  const normalizeScopeSet = raw =>
    Array.isArray(raw)
      ? [...new Set(raw.filter(item => typeof item === 'string').map(item => item.trim()).filter(Boolean))].sort()
      : [];
  const scopesMatch = (left, right) => {
    const normalizedLeft = normalizeScopeSet(left);
    const normalizedRight = normalizeScopeSet(right);
    return (
      normalizedLeft.length === normalizedRight.length && normalizedLeft.every((scope, index) => scope === normalizedRight[index])
    );
  };
  ...
  if (!['admin', 'developer', 'operator', 'auditor', 'viewer', 'user'].includes(role))
    throw new Error('Invalid OAuth role');
  const scopes = input.scopes ?? existing.scopes ?? [];
  const roleProvided = hasOwn(input, 'role');
  const scopesProvided = hasOwn(input, 'scopes');
  const roleChanged = roleProvided && input.role !== (existing.role ?? 'viewer');
  const existingScopes = existing.scopes ?? [];
  const scopesChanged = scopesProvided && !scopesMatch(scopes, existingScopes);
  if (
    !Array.isArray(scopes) ||
    !scopes.every(scope => typeof scope === 'string' && /^[a-z0-9.*:_-]{1,128}$/i.test(scope))
  )
    throw new Error('Invalid OAuth scopes');
  if (role !== 'admin' && scopes.includes('*')) {
    if (roleChanged || scopesChanged) {
      throw new Error('Wildcard OAuth scope is restricted to administrators');
    }
    // If an existing non-admin mapping was previously provisioned with a wildcard
    // scope, allow updating unrelated authorization fields (such as requireApproval)
    // without forcing an immediate migration.
    if (!Array.isArray(existingScopes) || !existingScopes.includes('*')) {
      throw new Error('Wildcard OAuth scope is restricted to administrators');
    }
  }
  ...
  const roleDefaultRequireApproval = role === 'admin' ? false : true;
  const requireApproval = input.requireApproval ?? existing.requireApproval ?? roleDefaultRequireApproval;
  ...
  return Object.fromEntries(
    Object.entries(clients).map(([clientId, override]) => {
      ...
      if (
        ...
        !['admin', 'developer', 'operator', 'auditor', 'viewer', 'user'].includes(override.role)
      )
  ...
  if (parsed?.users) {
    for (const [username, data] of Object.entries(parsed.users)) {
      ...
      users.push({
        ...
        requireApproval: mapping.requireApproval ?? ((mapping.role || 'viewer') === 'admin' ? false : true),
...
export async function getOAuthClients() {
  ...
  let parsed;
  try {
    ({ parsed } = parseAutheliaConfig(raw));
  } catch (error) {
    return [];
  }
...
export async function addOAuthClient({ clientId, clientName, redirectUris }) {
  ...
  const { parsed, templates } = parseAutheliaConfig(raw);
  ...
  const updatedConfig = dumpAutheliaConfig(parsed, templates);
