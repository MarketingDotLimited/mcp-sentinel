...
function currentKey() {
  const configuredKey = loadCredentialSecret('CONTROL_PLANE_KEY', 'state-key');
  const keyId = process.env.CONTROL_PLANE_KEY_ID || 'state-v1';
  let keyHex = configuredKey;
  if (!keyHex) {
    try {
      keyHex = createKeyProvider().get(keyId).value;
    } catch (error) {
...
function keyForId(keyId) {
  ...
  if (process.env.MCP_KEY_PROVIDER && process.env.MCP_KEY_PROVIDER !== 'local') {
    const external = createKeyProvider().get(keyId).value;
    if (!/^[a-f0-9]{64}$/i.test(external)) throw new Error(`Archived state key '${keyId}' is invalid`);
    return Buffer.from(external, 'hex');
  }
