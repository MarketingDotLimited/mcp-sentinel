...
const SCHEMAS = {
  ...
  get_my_ssh_access: object({
    sshPolicyVersion: z.number().int().nonnegative(),
    identityPreference: z.record(z.unknown()).nullable(),
    subjectClientPreference: z.record(z.unknown()).nullable(),
    ownedConnections: records,
    project: z.record(z.unknown()).optional(),
  }),
  set_my_ssh_access: object({
    scope: z.enum(['identity', 'current-client', 'connection']),
    sshEnabled: z.boolean(),
    sshAllowed: z.boolean().nullable(),
    sshPolicyVersion: z.number().int().positive(),
  }),
  list_ssh_access_policies: object({
    sshPolicyVersion: z.number().int().nonnegative(),
    globalAndScoped: records,
    hosts: records,
    connections: records,
    projects: records,
    identities: records,
    oauthClients: records,
    subjectClients: records,
    history: records,
  }),
  admin_set_ssh_access: object({
    targetType: z.string(),
    targetId: z.string(),
    sshAllowed: z.boolean().nullable(),
