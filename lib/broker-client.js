...
function requestViaSocket(socketPath, operation, parameters, { timeoutMs, signal } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      const timedOut = new Error(`Privilege broker request timed out on ${socketPath}`);
      timedOut.code = 'E_BROKER_UNAVAILABLE';
      timedOut.socketPath = socketPath;
      finish(timedOut);
    }, timeoutMs);
    ...
    signal?.addEventListener(
      ...
      () => {
        ...
        cancelled.code = 'E_BROKER_UNAVAILABLE';
        cancelled.socketPath = socketPath;
