...
describe('security.js OIDC validation', async () => {
  ...
  before(async () => {
    ...
    fs.writeFileSync(path.join(tmpDir, 'mappings.json'), JSON.stringify({
      testuser: { linuxUser: 'myuser', role: 'developer', scopes: ['files.*'], clients: {
        'client-id-1': { linuxUser: 'myuser-1', role: 'admin', scopes: ['*'] }
      }},
      incompleteuser: { linuxUser: 'root', clients: { 'client-id-1': { linuxUser: 'root' } } },
      badroleuser: { linuxUser: 'user2', role: 'haxor', scopes: ['*'], clients: { 'client-id-1': { linuxUser: 'user2', role: 'haxor', scopes: ['*'] } } }
    }));
      ...
      namedExports: {
        ...
        jwtVerify: async (token, jwks, options) => {
          ...
          if (token === 'missing-client-mapping') {
             return { payload: { ...basePayload, azp: 'bad-client', preferred_username: 'testuser' }, protectedHeader: { typ: 'jwt' } };
          }
          if (token === 'root-mapping') {
             return { payload: { ...basePayload, preferred_username: 'incompleteuser' }, protectedHeader: { typ: 'jwt' } };
          }
          if (token === 'bad-role') {
  ...
  async function checkToken(token) {
    ...
    let statusCalled = 0, nextCalled = false;
    const res = { 
  ...
  it('OIDC loadUserMappings throws', async () => {
    const { DatabaseSync } = await import('node:sqlite'); const db = new DatabaseSync(process.env.MCP_STATE_DB); db.exec('DROP TABLE oauth_mappings'); db.close();
