...
test('docker tools', async t => {
  ...
  await t.test('runSandboxedCode throws on unsupported language', async () => {
    await assert.rejects(docker.runSandboxedCode({ language: 'go', code: 'print("hi")' }), /Unsupported language: go/);
  });

  await t.test('runSandboxedCode throws on missing/empty code', async () => {
    await assert.rejects(docker.runSandboxedCode({ language: 'python', code: '' }), /code is required/);
    await assert.rejects(docker.runSandboxedCode({ language: 'python', code: null }), /code is required/);
  });
  ...
  await t.test('runSandboxedCode throws on excessive file size', async () => {
    await assert.rejects(
      docker.runSandboxedCode({
        language: 'python',
        code: 'print("hi")',
        files: { 'file.txt': 'a'.repeat(1024 * 1024 + 1) },
      }),
  ...
  await t.test('runSandboxedCode successful execution with docker runtime', async () => {
    process.env.SANDBOX_RUNTIME = 'docker';
    process.env.SANDBOX_ALLOW_DOCKER = 'true';
    setupSpawnMock({ stdoutStr: 'hello docker' });
    const result = await docker.runSandboxedCode({ language: 'python', code: 'print("hello docker")' });
    assert.equal(result.success, true);
    assert.equal(result.stdout, 'hello docker');
  });
