...
describe('router.js', () => {
  let dom;
  beforeEach(() => {
    dom = new JSDOM('<!DOCTYPE html><html><body><div id="main-content"></div><nav id="sidebar" style="display:none"></nav><button id="mobile-toggle" hidden></button><a class="nav-link" href="#/dashboard"></a><a class="nav-link" href="#/login"></a></body></html>', { url: 'http://localhost' });
  ...
  test('Router basic', async (t) => {
    ...
    const dummyPage = {
      render: (c) => { pageRendered = true; c.textContent = 'dummy'; },
      destroy: () => { pageDestroyed = true; }
    };
