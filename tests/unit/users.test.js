...
test('users tools', async t => {
  ...
  await t.test('getUserInfo allows user to view their own info or admin to view any', async () => {
    await assert.rejects(
      users.getUserInfo({ username: 'otheruser' }, userIdentity),
      /You can only view your own user info/
    );
  ...
  await t.test('createUser requires admin and valid inputs', async () => {
    await assert.rejects(
      users.createUser({ username: 'newuser' }, userIdentity),
      /User management requires admin role/
    );

    await assert.rejects(users.createUser({ username: 'root' }, adminIdentity), /Cannot modify reserved system user/);
    await assert.rejects(
      users.createUser({ username: 'newuser', groups: 'invalid group' }, adminIdentity),
      /groups must contain valid group names/
    );

    await users.createUser(
      { username: 'newuser', groups: 'group1,group2', shell: '/bin/bash', comment: 'test user', createHome: false },
      adminIdentity
    );
  ...
  await t.test('setUserPassword requires admin and valid password', async () => {
    ...
    await assert.rejects(
      users.setUserPassword({ username: 'user1', password: 'password\r1234' }, adminIdentity),
      /Password contains invalid characters/
    );
    await assert.rejects(
      users.setUserPassword({ username: 'user1', password: 'password\x001234' }, adminIdentity),
      /Password contains invalid characters/
    );
    await assert.rejects(
      users.setUserPassword({ username: 'user1', password: 'password:1234' }, adminIdentity),
      /Password contains invalid characters/
    );
    await assert.rejects(
      users.setUserPassword({ username: 'user1', password: 'a'.repeat(1025) }, adminIdentity),
      /Password must be 12-1024 characters/
    );
    await assert.rejects(
      users.setUserPassword({ username: 'user1', password: null }, adminIdentity),
      /Password contains invalid characters/
    );
  ...
  await t.test('modifyUser requires admin and valid inputs', async () => {
    ...
    await assert.rejects(
      users.modifyUser({ username: 'user1', lockAccount: true, unlockAccount: true }, adminIdentity),
      /Cannot lock and unlock account simultaneously/
    );
    await assert.rejects(
      users.modifyUser({ username: 'user1', expireDate: '2023/01/01' }, adminIdentity),
      /expireDate must be YYYY-MM-DD or empty/
    );
    ...
    assert.deepEqual(brokerCallMock.mock.calls[1].arguments, [
      'user.modify',
      {
        username: 'user1',
        unlockAccount: true,
  ...
  await t.test('manageSshKeys allows user to manage own or admin any', async () => {
    await assert.rejects(
      users.manageSshKeys({ username: 'otheruser', action: 'list' }, userIdentity),
      /You can only manage your own SSH keys/
    );
    await assert.rejects(
      users.manageSshKeys({ username: 'normaluser', action: 'invalid' }, userIdentity),
      /Invalid SSH key action/
    );
  ...
  await t.test('manageSshKeys validates public key on add', async () => {
    ...
    await assert.rejects(
      users.manageSshKeys({ username: 'user1', action: 'add', publicKey: invalidKeyType }, adminIdentity),
      /Invalid SSH public key payload/
    );
    await assert.rejects(
      users.manageSshKeys({ username: 'user1', action: 'add', publicKey: invalidKeyBase64 }, adminIdentity),
      /Invalid SSH public key payload/
    );
    ...
    assert.deepEqual(brokerCallMock.mock.calls[0].arguments, [
      'user.ssh',
      { username: 'user1', action: 'add', publicKey: 'ssh-rsa AAAAB3NzaC1yc2EAAA user@host' },
    ]);
  });
  ...
  await t.test('manageSshKeys passes keyIndex on remove', async () => {
    ...
    assert.deepEqual(brokerCallMock.mock.calls[0].arguments, [
      'user.ssh',
      { username: 'user1', action: 'remove', keyIndex: 1 },
    ]);
  });
});
