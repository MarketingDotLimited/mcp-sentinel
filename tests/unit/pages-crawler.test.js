    ...
    for (const file of files) {
      ...
      const componentName =
        file
          ...
          .join('') + 'Page';
      const component =
        mod[componentName] || mod.default || window[componentName] || window[componentName.replace('Oauth', 'OAuth')];
      ...
      const runPass = async scenario => {
        ...
        Object.defineProperty(global, 'navigator', {
          value: {
            clipboard: {
              writeText: async () => {
                if (scenario.copyFail) throw new Error('fail');
              },
            },
          },
          configurable: true,
          ...
          return {
            ...
            json: async () => {
              if (url.includes('security'))
                return { status: 'safe', checks: isPop ? [{ id: 'c1', status: 'pass', message: 'ok' }] : [] };
              if (url.includes('approvals'))
                return {
                  approvals: isPop
                    ? [{ id: 'r1', status: 'pending', createdAt: Date.now(), requestedBy: { userId: 'u1' } }]
                    : [],
                };
              if (url.includes('projects')) return { projects: isPop ? [{ id: 'p1', name: 'p', repoPath: '/p' }] : [] };
              if (url.includes('workflows'))
                return {
                  workflows: isPop ? [{ title: 'w', description: 'd', risk: 'read-only', prompts: ['p1'] }] : [],
                };
              if (url.includes('teams')) return { teams: isPop ? [{ id: 't1', name: 't', members: [] }] : [] };
              if (url.includes('backups'))
                return { backups: isPop ? [{ timestamp: Date.now(), user: 'u', size: 1024 }] : [] };
              if (url.includes('logs'))
                return { logs: isPop ? [{ id: 'l1', message: 'm', timestamp: Date.now() }] : [] };
              if (url.includes('administration')) return { config: { admin: true } };
              if (url.includes('sessions'))
                return { sessions: isPop ? [{ id: 's1', username: 'u1', started: Date.now() }] : [] };
              if (url.includes('action-manifest'))
                return {
                  manifest: {
                    version: 1,
                    hash: 'abc',
                    tools: isPop
                      ? [{ name: 'tool1', description: 'desc', annotations: {}, inputSchema: {}, outputSchema: {} }]
                      : [],
                  },
                  refreshChecklist: [],
                };
              if (url.includes('ssh-access'))
                return { rules: isPop ? [{ id: 'ssh1' }] : [], configuration: { port: 22 } };
              if (url.includes('capabilities'))
                return {
                  capabilities: isPop
                    ? [{ id: 'pack1', title: 'Pack 1', description: 'Desc', enabled: true, defaultEnabled: false }]
                    : [],
                };
              if (url.includes('connect')) return { connection: { status: 'ok' } };
              if (url.includes('keys')) return { keys: isPop ? [{ id: 'k1', prefix: 'abc', name: 'key1' }] : [] };
              if (url.includes('oauth'))
                return { providers: isPop ? [{ id: 'prov' }] : [], clients: isPop ? [{ id: 'c1' }] : [], mappings: [] };
              return [];
            },
          };
        });
        ...
        for (const el of elements) {
          if (el.tagName === 'FORM') {
            try {
              el.dispatchEvent(new dom.window.Event('submit', { cancelable: true }));
            } catch (e) {
              void e;
            }
