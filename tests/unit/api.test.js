...
describe('api.js', () => {
  ...
  test('API.request non-ok status', async (t) => {
    ...
    global.fetch = t.mock.fn(async () => ({
      status: 400, ok: false, json: async () => ({ error: 'Bad request', code: 'ERR_BAD', resolution: 'Fix it', detail: 'More info' })
    }));
    try { await API.get('/test'); } catch (e) {
    ...
    global.fetch = t.mock.fn(async () => {
      const err = new Error();
      err.json = async () => { throw new Error('Parse error'); };
      err.text = async () => { throw new Error('Text parse error'); };
      err.ok = false;
      err.status = 500;
      return err;
    });
    try { await API.get('/test2'); } catch (e) {
      assert.match(e.message, /Unable to parse API response/);
    }
    
    // Broker error code
    global.fetch = t.mock.fn(async () => ({
      status: 500, ok: false, json: async () => ({ error: 'Broker error', code: 'BROKER_UNAVAILABLE' })
    }));
    try { await API.get('/test3'); } catch (e) {
      assert.strictEqual(e.status, 503);
    }

    // State error text
    global.fetch = t.mock.fn(async () => ({
      status: 500, ok: false, json: async () => ({ error: 'database is locked' })
    }));
    try { await API.get('/test4'); } catch (e) {
      assert.strictEqual(e.status, 503);
    }
  });
  ...
  test('API.request fallbacks', async (t) => {
    ...
    try { await API.get('/non-fallback'); } catch (e) {
