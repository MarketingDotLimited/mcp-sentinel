...
describe('All Pages Test', () => {
  ...
  test('Crawl all pages with API mocked', async (t) => {
    ...
    const files = fs.readdirSync(pagesDir).filter(f => f.endsWith('.js') && f !== 'dashboard.js' && f !== 'action-manifest.js');
    ...
    for (const file of files) {
      ...
      const successData = {
        ...
        providers: [], clients: [], mappings: [],
        rules: [{ id: 'r1', principle: 'p', environment: 'e' }], configuration: { max_age: 100 },
      ...
      // Pass 2: Error
      API.get = t.mock.fn(async () => { throw new Error('API Error Mock'); });
      API.post = t.mock.fn(async () => { throw new Error('API Error Mock'); });
      API.put = t.mock.fn(async () => { throw new Error('API Error Mock'); });
      API.delete = t.mock.fn(async () => { throw new Error('API Error Mock'); });
