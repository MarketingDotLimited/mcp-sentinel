...
describe('security.js additional coverage', async () => {
  ...
  it('addApiKey with bad role or bad unix user', async () => {
    ...
    await assert.rejects(security.addApiKey('mcp_' + 'a'.repeat(64), { role: 'viewer', userId: 'nonexistentuser999' }), /Invalid user/);
  });
  ...
  it('ipWhitelist middleware', () => {
    ...
    security.ipWhitelist({ path: '/api', socket: { remoteAddress: '1.2.3.4' }, headers: {} }, {}, () => { nextCalled = true; });
  ...
  it('authenticate middleware - global ALLOWED_IPS branch & invalid IPs', async () => {
    ...
    const resInvalid = { status: (c) => ({ json: (b) => { return b; } }) };
    ...
    let nextCalled = false;
    security.authenticate(reqValid, {}, () => { nextCalled = true; });
    assert.equal(nextCalled, true);
    ...
    security.authenticate(reqValidPlain, {}, () => { nextCalled2 = true; });
    ...
    const resBadIp = { status: (c) => ({ json: (b) => { return b; } }) };
  ...
  it('sendUnauthorized without OAUTH_RESOURCE_URL', () => {
    ...
    const res = { 
      status: (c) => ({ json: (b) => b }),
      set: (n, v) => { res.headers = res.headers || {}; res.headers[n] = v; }
    };
    security.authenticate(req, res, () => {});
    assert.equal(res.headers['WWW-Authenticate'], 'Bearer resource_metadata="http://localhost:3000/.well-known/oauth-protected-resource"');
  });
  ...
  it('authenticateJWT with fallback for old mcp_ format', () => {
    ...
    const result = security.authenticateJWT(req, res, () => { called = true; });
  ...
  it('persistJwtRevocations sqlite successful commit', () => {
    ...
    const res = { json: () => {} };
    security.revokeSessionToken({ identity: { authType: 'apiKey', jti: 'test-commit', tokenExpiresAt: Date.now() + 10000 }, clientIP: '127.0.0.1' }, res);
  });
  ...
  it('authenticateJWT no Bearer prefix', () => {
    ...
    let statusCalled = null; const res2 = { status: (c) => { statusCalled = c; return { json: (b) => { console.log("DENIED:" + JSON.stringify(b)); } }; } }; security.authenticateJWT(req, res2, () => { nextCalled = true; });
  ...
  it('authenticateJWT stepUp missing binding', () => {
    ...
    const token = jwt.sign({
      ...
      stepUp: true,
      jti: 'bad-stepup'
    }, process.env.JWT_SECRET, { algorithm: 'HS256', issuer: 'mcp-server', audience: 'mcp-client' });
    const req = { headers: { authorization: 'Bearer ' + token }, socket: { remoteAddress: '127.0.0.1' } };
    let statusCalled = 0;
    const res = { status: (c) => { statusCalled = c; return res; }, json: () => res, set: () => res };
  ...
  it('authenticateJWT IP mismatch', async () => {
    ...
    let nextCalled = false;
    const res = { status: () => res, json: () => res, set: () => res }; let statusCalled = null; const res2 = { status: (c) => { statusCalled = c; return { json: (b) => { console.log("DENIED:" + JSON.stringify(b)); } }; } }; security.authenticateJWT(req, res2, () => { nextCalled = true; });
    assert.equal(nextCalled, true);
  });
});
