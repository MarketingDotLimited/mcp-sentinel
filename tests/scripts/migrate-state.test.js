...
test('migrate-state.js', async t => {
  ...
  await t.test('handles missing legacy file by exiting gracefully in dry-run', async () => {
    ...
    try {
      ...
      if (e.message !== 'MOCK_EXIT') throw e;
    }
  ...
  await t.test('throws when missing legacy file in apply mode', async () => {
    ...
    await assert.rejects(
      ...
      /CONTROL_PLANE_STATE_FILE must identify the legacy control-plane JSON/
    );
  });
        ...
        readFile: async f => {
          ...
          if (f.endsWith('verified.json'))
            return JSON.stringify({
              output: '/mock/export/test.json',
              sha256: 'fake',
              counts: { automations: 1, fleets: 0, backupTargets: 0, webhooks: 0 },
            });
          if (f.endsWith('test.json'))
            return JSON.stringify({ counts: { automations: 1, fleets: 0, backupTargets: 0, webhooks: 0 } });
          return '';
        },
      },
    });
  ...
  await t.test('throws on count mismatch in apply mode', async () => {
    ...
    mock.module('node:fs/promises', {
      ...
      defaultExport: {
        readFile: async f => {
          if (f.endsWith('legacy.json'))
            return '{"projects": [{"id":"00000000-0000-1000-8000-000000000000","rootPath":"x"}]}';
          return '';
        },
      },
    });
  ...
  await t.test('throws on missing aliases in apply mode', async () => {
    ...
    mock.module('../../lib/sqlite-state.js', {
      ...
      defaultExport: {
        ...
        loadSqliteState: () => ({
          approvals: [],
          projects: [{ id: 'new', rootPath: 'x' }],
          organizations: [],
          teams: [],
        }), // count is 1, but no legacyIds aliases!
      },
    });
  ...
  await t.test('throws on integrity check failure in apply mode', async () => {
    ...
    await assert.rejects(
      () => import(`${SCRIPT_PATH}?t=${Date.now()}`),
      /Migrated SQLite database failed integrity verification/
    );
  });
});
