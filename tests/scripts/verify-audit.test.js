...
test('verify-audit.js', async t => {
  ...
  await t.test('throws on previous hash mismatch', async () => {
    ...
    await assert.rejects(() => import(`${SCRIPT_PATH}?t=${Date.now()}`), /Audit previous-hash mismatch in audit-1.log/);
  });
  ...
  await t.test('throws on HMAC mismatch', async () => {
    ...
    await assert.rejects(() => import(`${SCRIPT_PATH}?t=${Date.now()}`), /Audit HMAC mismatch in audit-1.log/);
  });
