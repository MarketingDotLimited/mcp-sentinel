process.on("unhandledRejection", err => console.error("UR:", err)); process.on("uncaughtException", err => console.error("UE:", err));
...
const fsMock = {
    readFileSync: mock.fn(() => ''), mkdirSync: mock.fn(), chownSync: mock.fn(), chmodSync: mock.fn(), lstatSync: mock.fn(() => ({ isFile: () => true, isDirectory: () => true, isSymbolicLink: () => false, mode: 0o600, uid: 0, gid: 0, size: 1024 })),
     writeFileSync: mock.fn(), existsSync: mock.fn(), copyFileSync: mock.fn(),
...
if (!runCommand) runCommand = (await import(SCRIPT_PATH)).runCommand;
    ...
    const run = async (action, arg = null, expectedCode = 0) => {
        ...
        const actualCode = process.exitCode; console.error(`>>> TEST ${action} ${arg} FINISHED WITH ${actualCode} <<<`); console.log("ACTUAL CODE FOR", action, arg, "IS", actualCode);
        process.exitCode = 0;
...
await t.test('prepare-not-root', async () => {
    ...
    await t.test('stage-bad-file-size', async () => {
        setupStageHappy();
        fsMock.lstatSync.mock.mockImplementation(() => ({ isFile: () => true, isDirectory: () => true, isSymbolicLink: () => false, mode: 0o600, uid: 0, gid: 0, size: 2 * 1024 * 1024 * 1024 }));
        await run('stage', 'a.tar.gz', 1);
    });
    ...
    await t.test('stage-bad-parse-signature', async () => {
        setupStageHappy();
        libMock.validateReleaseManifest.mock.mockImplementation(() => '1.0.0-abcdef123456'); libMock.parseValidSignatureFingerprint.mock.mockImplementation(() => { throw new Error('parse error'); });
        await run('stage', 'a.tar.gz', 1);
    });
