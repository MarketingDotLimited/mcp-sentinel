...
test('rotate-state-key.js', async (t) => {
    ...
    await t.test('throws if new key ID missing', async () => {
        ...
        await assert.rejects(
            () => import(`${SCRIPT_PATH}?t=${Date.now()}`),
            /CONTROL_PLANE_KEY_ID must name the new key/
        );
    });
    ...
    await t.test('throws if new key invalid', async () => {
        ...
        await assert.rejects(
            () => import(`${SCRIPT_PATH}?t=${Date.now()}`),
            /Current state-key credential is invalid/
        );
    });
    ...
    await t.test('rotates keys for tables', async () => {
        ...
        db.prepare('INSERT INTO approvals (id, payload) VALUES (?, ?)').run('app-1', JSON.stringify({
            some_field: 'public',
        ...
        const decipher = crypto.createDecipheriv('aes-256-gcm', Buffer.from(newKeyHex, 'hex'), Buffer.from(parsed.nested[0].iv, 'base64'));
        decipher.setAuthTag(Buffer.from(parsed.nested[0].tag, 'base64'));
        const plaintext = Buffer.concat([decipher.update(Buffer.from(parsed.nested[0].ciphertext, 'base64')), decipher.final()]);
    ...
    await t.test('handles rollback on error', async (t) => {
        ...
        await assert.rejects(
            () => import(`${SCRIPT_PATH}?t=${Date.now()}`),
            /Invalid stored key ID 'invalid-id!'/
        );
    });
