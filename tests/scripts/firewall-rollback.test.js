...
test('firewall-rollback.js', async t => {
  ...
  await t.test('throws if ID is invalid', async () => {
    ...
    await assert.rejects(() => import(`${SCRIPT_PATH}?t=${Date.now()}`), /Invalid firewall rollback ID/);
  });
  ...
  await t.test('throws if snapshot is not a directory', async t => {
    ...
    await assert.rejects(() => import(`${SCRIPT_PATH}?t=${Date.now()}`), /Invalid firewall snapshot/);
  });
  ...
  await t.test('executes rollback successfully', async t => {
    ...
    assert.deepEqual(execFileSyncArgs, ['/usr/sbin/ufw', ['reload'], { timeout: 30000, stdio: 'inherit' }]);
