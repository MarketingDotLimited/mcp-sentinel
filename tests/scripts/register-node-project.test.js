...
test('register-node-project.js', async t => {
  ...
  t.mock.method(process.stderr, 'write', data => {
  ...
  await t.test('validateNodeProject validates input', async t => {
    ...
    mock.module('node:child_process', {
      ...
      defaultExport: { execFileSync: () => cpExecMock },
    });
    ...
    await assert.rejects(
      () => module.validateNodeProject({ id: '00000000-0000-1000-8000-000000000000', name: '' }),
    ...
    await assert.rejects(
      () =>
        module.validateNodeProject({
          id: '00000000-0000-1000-8000-000000000000',
          name: 'valid',
          rootPath: '/a',
          repoPath: '/b',
        }),
      /Project repository must be inside its root/
    );
    await assert.rejects(
      () =>
        module.validateNodeProject({
      ...
      /Project execution user must be non-root/
    );
    ...
    await assert.rejects(
      ...
      /Project execution user must resolve to a non-root UID/
    );
    ...
    await assert.rejects(
      () =>
        module.validateNodeProject({
          ...
          repoPath: '/a',
          runAsUser: 'user',
          permittedTasks: ['invalid'],
        }),
      /Project contains an invalid task recipe/
    );
    await assert.rejects(
      () =>
        module.validateNodeProject({
          ...
          runAsUser: 'user',
          permittedTasks: ['npm'],
          permittedGitActions: ['invalid'],
        }),
      /Project contains an invalid Git recipe/
    );
    await assert.rejects(
      () =>
        module.validateNodeProject({
          ...
          runAsUser: 'user',
          permittedTasks: ['npm'],
          permittedGitActions: ['status'],
          testNetworkHosts: ['not-an-ip'],
        }),
      /Project test network dependencies must be explicit IP addresses/
    );
  ...
  await t.test('main() dry-run', async () => {
    ...
    mock.module('node:child_process', {
      ...
      defaultExport: { execFileSync: () => '1000' },
    });
    const { main } = await import(`${SCRIPT_PATH}?t=${Date.now()}`);
    try {
      await main();
  ...
  await t.test('main() apply creates entry', async () => {
    process.argv = [
      'node',
      'scripts/register-node-project.js',
      '--project',
      '/mock/project.json',
      '--apply',
      '--confirm',
