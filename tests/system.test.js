...
mock.module('fs/promises', {
  namedExports: {
    stat: async (p) => {
       ...
       if (p === '/fake/real-project' || p === '/fake/real-allowed' || p === '/fake/eacces-project' || p === '/fake/wrong-env' || p === '/fake/wrong-db' || p === '/fake/missing-env' || p === '/fake/fallback-db') return { isDirectory: () => true };
    ...
    realpath: async (p) => {
       ...
       if (p === '/fake/real-project' || p === '/fake/real-allowed' || p === '/fake/real-project/t1' || p === path.resolve('/fake/real-project/t1') || p === '/fake/eacces-project' || p === '/fake/wrong-env' || p === '/fake/wrong-db' || p === '/fake/missing-env' || p === '/fake/fallback-db' || p === '/fake/file-project') return p;
...
mock.module('../lib/project-operation-dispatcher.js', {
  namedExports: {
    dispatchProjectOperation: async (projectId, identity, operation, payload, options) => {
      ...
      if (payload.runner === 'artisan' && payload.target === 'fail_with_stderr') return { exitCode: 1, stderr: 'Custom error' };
      if (payload.runner === 'artisan' && payload.target === 'exit_2') return { exitCode: 2 };
      if (payload.runner === 'artisan' && payload.target === 'unsafe') return { exitCode: 1, stderr: '.env.testing is missing' };
      if (payload.runner === 'artisan') return { exitCode: 1 };
      if (payload.runner === 'npm' && payload.target === 'timeout') {
         const err = new Error('timeout'); err.code = 'ETIMEDOUT'; err.stdout = 'A'.repeat(1048577); throw err;
      }
...
mock.module('../lib/control-plane.js', {
  namedExports: {
    ...
    loadTaskRunState: async (runId) => {
      if (runId === 'found-db') return { runId: 'found-db', projectId: 'p1', state: 'completed', stdout: 'loaded', stderr: '', exitCode: 0, owner: JSON.stringify({ subject: 'u1', clientId: null, userId: 'u1' }) };
      return null;
    },
    resolveProjectTransport: async (projectId) => {
       if (projectId === 'p2') return { kind: 'ssh-gateway', project: { id: 'p2', runAsUser: 'u1', permittedTasks: ['unknown_runner', 'npm'], allowFullSuite: true } };
       if (projectId === 'p3') return { kind: 'local', project: { id: 'p3', runAsUser: 'u1', permittedTasks: ['npm'], repoPath: '/fake/requested', allowFullSuite: true } };
       if (projectId === 'p4') return { kind: 'local', project: { id: 'p4', runAsUser: 'u1', rootPath: '/fake/requested' } }; // no permittedTasks
       if (projectId === 'p10') return { project: { id: 'p10', permittedTasks: ['npm'] } }; // no transport, no runAsUser
       if (projectId === 'p11') return { kind: 'local', project: { id: 'p11', runAsUser: 'u1', permittedTasks: ['artisan'], testDatabase: 'fake_db', rootPath: '/fake/wrong-env', allowFullSuite: true } };
       if (projectId === 'p12') return { kind: 'local', project: { id: 'p12', runAsUser: 'u1', permittedTasks: ['artisan'], testDatabase: 'fake_db', rootPath: '/fake/wrong-db', allowFullSuite: true } };
       if (projectId === 'p13') return { kind: 'local', project: { id: 'p13', runAsUser: 'u1', permittedTasks: ['artisan'], testDatabase: 'fake_db', rootPath: '/fake/missing-env', allowFullSuite: true } };
       if (projectId === 'p14') return { kind: 'local', project: { id: 'p14', runAsUser: 'u1', permittedTasks: ['artisan'], rootPath: '/fake/real-allowed', allowFullSuite: true } };
       if (projectId === 'p15') return { kind: 'local', project: { id: 'p15', runAsUser: 'u1', permittedTasks: ['npm'], allowFullSuite: true } }; // no rootPath/repoPath
       if (projectId === 'p16') return { kind: 'local', project: { id: 'p16', runAsUser: 'u1', permittedTasks: ['npm'], rootPath: '/fake/file-project', allowFullSuite: true } };
       if (projectId === 'p17') return { kind: 'local', project: { id: 'p17', runAsUser: 'u1', permittedTasks: ['artisan'], testDatabase: 'fake_db', rootPath: '/fake/fallback-db', allowFullSuite: true } };
       if (projectId === 'p5') return { kind: 'local', project: { id: 'p5', runAsUser: 'u1', permittedTasks: ['artisan'], testDatabase: 'fake_db', rootPath: '/fake/requested', allowFullSuite: true } };
       if (projectId === 'p6') return { kind: 'local', project: { id: 'p6', runAsUser: 'u1', permittedTasks: ['npm'], rootPath: '/fake/requested' } }; // no allowFullSuite
       if (projectId === 'p7') return { kind: 'local', project: { id: 'p7', runAsUser: 'u1', permittedTasks: ['python'], rootPath: '/fake/requested', allowFullSuite: true } };
       if (projectId === 'p8') return { kind: 'local', project: { id: 'p8', runAsUser: 'u1', permittedTasks: ['rust'], rootPath: '/fake/requested', allowFullSuite: true } };
       if (projectId === 'p9') return { kind: 'local', project: { id: 'p9', runAsUser: 'u1', permittedTasks: ['artisan'], testDatabase: 'fake_db', rootPath: '/fake/eacces-project', allowFullSuite: true } };
       return { kind: 'local', project: { id: 'p1', runAsUser: 'u1', permittedTasks: ['npm', 'artisan'], allowFullSuite: true, rootPath: '/fake/requested' } };
    },
    persistTaskRun: async () => ({})
  }
});

const { getSystemInfo, getProcesses, killProcess, startProjectTestRun, cancelProjectTestRun, buildProjectTestInvocation, pruneProjectTestRuns, getProjectTestRun, runProjectTests } = await import('../tools/system.js');
...
describe('system tools', () => {
  ...
  it('startProjectTestRun throws if recipe is not permitted', async () => {
    await assert.rejects(
       startProjectTestRun({ confirm: true, runner: 'unknown', projectId: 'p1' }, { role: 'user', userId: 'u1' }, secureExec),
       /This test recipe is not permitted for the project/
    );
    // Cover missing permittedTasks fallback
    await assert.rejects(
       startProjectTestRun({ confirm: true, runner: 'npm', projectId: 'p4' }, { role: 'user', userId: 'u1' }, secureExec),
       /This test recipe is not permitted for the project/
    );
  });
  ...
  it('startProjectTestRun with laravel missing testDatabase throws', async () => {
    await assert.rejects(
       startProjectTestRun({ confirm: true, runner: 'artisan', projectId: 'p1' }, { role: 'user', userId: 'u1' }, async () => ({})),
  ...
  it('startProjectTestRun with laravel fails with EACCES', async () => {
    realpathCallCount = 0;
    await assert.rejects(
       startProjectTestRun({ confirm: true, runner: 'artisan', projectId: 'p9' }, { role: 'user', userId: 'u1' }, async () => ({})),
       /EACCES/
    );
  });
  ...
  it('startProjectTestRun with laravel fails with ENOENT', async () => {
    realpathCallCount = 0;
    await assert.rejects(
       startProjectTestRun({ confirm: true, runner: 'artisan', projectId: 'p14' }, { role: 'user', userId: 'u1' }, async () => ({})),
  ...
  it('startProjectTestRun with laravel fails with wrong APP_ENV', async () => {
    realpathCallCount = 0;
    await assert.rejects(
       startProjectTestRun({ confirm: true, runner: 'artisan', projectId: 'p11' }, { role: 'user', userId: 'u1' }, async () => ({})),
       /.env.testing must set APP_ENV=testing/
    );

    // missing APP_ENV should succeed (or proceed to check DB)
    realpathCallCount = 0;
    const run = await startProjectTestRun({ confirm: true, runner: 'artisan', projectId: 'p13' }, { role: 'user', userId: 'u1' }, async () => ({ exitCode: 0, stdout: 'ok', stderr: '' }));
    await run.completion;

    // fallback DB (DATABASE_DATABASE) should succeed
    realpathCallCount = 0;
    const runFb = await startProjectTestRun({ confirm: true, runner: 'artisan', projectId: 'p17' }, { role: 'user', userId: 'u1' }, async () => ({ exitCode: 0, stdout: 'ok', stderr: '' }));
    await runFb.completion;

    // test filter branches
    await assert.rejects(
       startProjectTestRun({ confirm: true, runner: 'artisan', projectId: 'p5', filter: ' ' }, { role: 'user', userId: 'u1' }, async () => ({})),
       /filter must be 1-256 characters on one line/
    );
    await assert.rejects(
       startProjectTestRun({ confirm: true, runner: 'artisan', projectId: 'p5', filter: 'a'.repeat(257) }, { role: 'user', userId: 'u1' }, async () => ({})),
       /filter must be 1-256 characters on one line/
    );
    await assert.rejects(
       startProjectTestRun({ confirm: true, runner: 'artisan', projectId: 'p5', filter: 'a\nb' }, { role: 'user', userId: 'u1' }, async () => ({})),
       /filter must be 1-256 characters on one line/
    );
  });
  ...
  it('startProjectTestRun with laravel fails with wrong DB_DATABASE', async () => {
    realpathCallCount = 0;
    await assert.rejects(
       startProjectTestRun({ confirm: true, runner: 'artisan', projectId: 'p12' }, { role: 'user', userId: 'u1' }, async () => ({})),
  ...
  it('startProjectTestRun local execution filter varieties', async () => {
    // filterFlag
    let run = await startProjectTestRun({ confirm: true, runner: 'python', projectId: 'p7', filter: 'f1' }, { role: 'user', userId: 'u1' }, async () => ({ exitCode: 0, stdout: 'ok', stderr: '' }));
    await run.completion;
    
    // filterSeparator
    run = await startProjectTestRun({ confirm: true, runner: 'rust', projectId: 'p8', filter: 'f1' }, { role: 'user', userId: 'u1' }, async () => ({ exitCode: 0, stdout: 'ok', stderr: '' }));
    await run.completion;

    // filter supported normally
    realpathCallCount = 0;
    run = await startProjectTestRun({ confirm: true, runner: 'artisan', projectId: 'p5', filter: 'f1' }, { role: 'user', userId: 'u1' }, async () => ({ exitCode: 0, stdout: 'ok', stderr: '' }));
    await run.completion;

    // unsupported local throws
    await assert.rejects(
       startProjectTestRun({ confirm: true, runner: 'npm', filter: 'f1', projectId: 'p3' }, { role: 'user', userId: 'u1' }, async () => ({})),
  ...
  it('startProjectTestRun with target traversing outside throws', async () => {
    realpathCallCount = 0;
    await assert.rejects(
       startProjectTestRun({ confirm: true, runner: 'npm', projectId: 'p3', target: '../outside' }, { role: 'user', userId: 'u1' }, async () => ({})),
       /Test target must stay inside the project/
    );

    realpathCallCount = 0;
    await assert.rejects(
       startProjectTestRun({ confirm: true, runner: 'npm', projectId: 'p3', target: 'symlink-outside' }, { role: 'user', userId: 'u1' }, async () => ({})),
  ...
  it('buildProjectTestInvocation input validation', async () => {
    ...
    await assert.rejects(
       buildProjectTestInvocation({ projectPath: '/fake/requested', runner: 'npm', target: '/absolute', allowedRoots: ['/fake/requested'] }),
    ...
    await assert.rejects(
       buildProjectTestInvocation({ projectPath: '/fake/file-project', runner: 'npm', allowedRoots: ['/fake/file-project'] }),
       /Project test root must be a directory/
    );

    // 227 target not supported
    await assert.rejects(
       buildProjectTestInvocation({ projectPath: '/fake/requested', runner: 'composer-validate', target: 't1', allowedRoots: ['/fake/requested'] }),
  ...
  it('cancelProjectTestRun branch coverage', async () => {
    // 481 confirm !== true
    await assert.rejects(
       cancelProjectTestRun({ runId: 'fake' }, {}),
    ...
    await run.completion;
    const adminCancelled = await cancelProjectTestRun({ confirm: true, runId: run.runId }, { role: 'admin', userId: 'admin1' });
  ...
  it('startProjectTestRun branch coverage for confirm, runAsUser and missing transport', async () => {
    ...
    await assert.rejects(
       startProjectTestRun({ confirm: true, runner: 'npm', projectId: 'p10' }, { role: 'user', userId: 'u1' }, async () => ({})),
  ...
  it('runProjectTests resolves completion with default waitMs', async () => {
    ...
    const res = await runProjectTests(
       ...
       { role: 'user', userId: 'u1' },
       async () => { await new Promise(r => setTimeout(r, 10)); return { exitCode: 0, stdout: '1 tests passed, 2 assertions', stderr: '' }; }
    );
  ...
  it('startProjectTestRun throws if allowFullSuite is missing and no target', async () => {
    await assert.rejects(
       startProjectTestRun({ confirm: true, runner: 'npm', projectId: 'p6' }, { role: 'user', userId: 'u1' }, async () => ({})),
  ...
  it('runProjectTests resolves completion', async () => {
    ...
    const res = await runProjectTests(
       ...
       { role: 'user', userId: 'u1' },
       async () => { await new Promise(r => setTimeout(r, 10)); return { exitCode: 0, stdout: '1 tests passed', stderr: '' }; }
    );
  ...
  it('startProjectTestRun with ssh-gateway validates inputs', async () => {
    ...
    await assert.rejects(
       startProjectTestRun({ confirm: true, runner: 'unknown_runner', projectId: 'p2' }, { role: 'user', userId: 'u1' }, secureExec),
       /Unsupported test runner: unknown_runner/
    );

    // Filter not supported
    await assert.rejects(
       startProjectTestRun({ confirm: true, runner: 'npm', filter: 'f1', projectId: 'p2' }, { role: 'user', userId: 'u1' }, secureExec),
       /filter is not supported by the npm runner/
    );

    // Target not allowed (absolute path)
    await assert.rejects(
       startProjectTestRun({ confirm: true, runner: 'npm', target: '/absolute', projectId: 'p2' }, { role: 'user', userId: 'u1' }, secureExec),
       /Test target must be a registered relative path/
    );

    // Target with .. not allowed
    await assert.rejects(
       startProjectTestRun({ confirm: true, runner: 'npm', target: 'a/../b', projectId: 'p2' }, { role: 'user', userId: 'u1' }, secureExec),
       /Test target must be a registered relative path/
    );

    // No target and allowFullSuite !== true
    const p2_no_full = { project: { id: 'p2', runAsUser: 'u1', permittedTasks: ['npm'] }, transport: { kind: 'ssh-gateway' } };
    await assert.rejects(
       startProjectTestRun({ confirm: true, runner: 'npm', projectId: 'p2' }, { role: 'user', userId: 'u1' }, secureExec, async () => p2_no_full),
       /A relative target is required because this project does not permit full-suite execution/
    );
  });
  ...
  it('cancelProjectTestRun throws if not found', async () => {
    await assert.rejects(cancelProjectTestRun({ runId: 'not-found', confirm: true }, {}), /Test run not found or expired/);
  });
});
