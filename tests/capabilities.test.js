...
describe('capabilities.js edge cases', () => {
  ...
  it('setCapability validations', async () => {
    ...
    await assert.rejects(capabilities.setCapability('core-server-care', false), /Core capability packs must remain enabled/);
  });
  ...
  it('rolls back on database error in save', async () => {
    ...
    db.exec('CREATE TRIGGER fail_insert BEFORE INSERT ON capabilities BEGIN SELECT RAISE(ABORT, "aborting insert"); END;');
    db.close();
  ...
  it('throws on non-ENOENT error in legacy load', async () => {
    ...
    mock.method(fs, 'readFile', () => { throw new Error('mock legacy error'); });
  ...
  it('migrates from existing capabilities.json with unknown pack and false enabled in object', async () => {
    ...
    await fs.writeFile(process.env.MCP_CAPABILITIES_FILE, JSON.stringify({ 
  ...
  it('covers environment variable defaults', async () => {
    ...
    mock.method(fs, 'mkdir', () => { throw new Error('mock mkdir error'); });
