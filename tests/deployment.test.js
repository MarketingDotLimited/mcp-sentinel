...
EMPTY=
  ...
  await t.test('validateReleaseManifest', async (t) => {
    ...
    await t.test('throws on invalid manifest types', () => {
      assert.throws(() => deployment.validateReleaseManifest(null, 'app.tar.gz', 'b'.repeat(64)), /Invalid release manifest/);
      assert.throws(() => deployment.validateReleaseManifest([], 'app.tar.gz', 'b'.repeat(64)), /Invalid release manifest/);
      assert.throws(() => deployment.validateReleaseManifest('string', 'app.tar.gz', 'b'.repeat(64)), /Invalid release manifest/);
    });

    await t.test('throws on unknown fields', () => {
      assert.throws(() => deployment.validateReleaseManifest({ ...validManifest, extra: 1 }, 'app.tar.gz', 'b'.repeat(64)), /Release manifest contains unknown fields/);
      assert.throws(() => deployment.validateReleaseManifest({ artifact: 'app.tar.gz' }, 'app.tar.gz', 'b'.repeat(64)), /Release manifest contains unknown fields/);
    });

    await t.test('throws on invalid version', () => {
      assert.throws(() => deployment.validateReleaseManifest({ ...validManifest, version: '1.2' }, 'app.tar.gz', 'b'.repeat(64)), /Invalid release version/);
    });

    await t.test('throws on invalid commit', () => {
      assert.throws(() => deployment.validateReleaseManifest({ ...validManifest, commit: 'short' }, 'app.tar.gz', 'b'.repeat(64)), /Invalid release commit/);
    });

    await t.test('throws on invalid sha256 or hash mismatch', () => {
      assert.throws(() => deployment.validateReleaseManifest({ ...validManifest, sha256: 'short' }, 'app.tar.gz', 'b'.repeat(64)), /Release artifact hash does not match/);
      assert.throws(() => deployment.validateReleaseManifest(validManifest, 'app.tar.gz', 'c'.repeat(64)), /Release artifact hash does not match/);
    });

    await t.test('throws on artifact name mismatch', () => {
      assert.throws(() => deployment.validateReleaseManifest(validManifest, 'other.tar.gz', 'b'.repeat(64)), /Release artifact name does not match/);
    });
  });
  ...
  await t.test('validateSigningFingerprint', async (t) => {
    ...
    await t.test('throws if expected is invalid format', () => {
      assert.throws(() => deployment.validateSigningFingerprint('A'.repeat(40), 'invalid'), /Trusted release signing fingerprint must contain 40 or 64 hexadecimal characters/);
      assert.throws(() => deployment.validateSigningFingerprint('A'.repeat(40), 'A'.repeat(41)), /Trusted release signing fingerprint must contain 40 or 64 hexadecimal characters/);
    });

    await t.test('throws on untrusted key', () => {
      assert.throws(() => deployment.validateSigningFingerprint('B'.repeat(40), 'A'.repeat(40)), /Release signature was made by an untrusted key/);
    });
  });
  ...
  await t.test('parseValidSignatureFingerprint', async (t) => {
    ...
    await t.test('throws on missing valid signature', () => {
      assert.throws(() => deployment.parseValidSignatureFingerprint(''), /No valid release signature status was returned/);
    });

    await t.test('throws on invalid signing fingerprint', () => {
      assert.throws(() => deployment.parseValidSignatureFingerprint('[GNUPG:] VALIDSIG INVALID'), /Release signature status contains an invalid signing fingerprint/);
    });
  });
  ...
  await t.test('validateArchiveEntries', async (t) => {
    ...
    await t.test('throws if too many entries', () => {
      ...
      assert.throws(() => deployment.validateArchiveEntries(entries, '1.0.0'), /Release archive contains too many entries/);
    });

    await t.test('throws on unsafe entry', () => {
      assert.throws(() => deployment.validateArchiveEntries(['other-prefix/file'], '1.0.0'), /Unsafe release archive entry/);
      assert.throws(() => deployment.validateArchiveEntries(['/mcp-sentinel-1.0.0/file'], '1.0.0'), /Unsafe release archive entry/);
      assert.throws(() => deployment.validateArchiveEntries(['mcp-sentinel-1.0.0/\0'], '1.0.0'), /Unsafe release archive entry/);
    });

    await t.test('throws on path traversal', () => {
      assert.throws(() => deployment.validateArchiveEntries(['mcp-sentinel-1.0.0/../file'], '1.0.0'), /Release archive entry escapes its version root/);
    });
  });
  ...
  await t.test('validateArchiveListing', async (t) => {
    ...
    await t.test('throws on unsupported entry format', () => {
      assert.throws(() => deployment.validateArchiveListing(['invalid format']), /Release archive contains an unsupported entry or listing format/);
    });

    await t.test('throws on invalid file size', () => {
      assert.throws(() => deployment.validateArchiveListing(['-rw-r--r-- user/group -100 2020-01-01 file']), /Release archive contains an unsupported entry/);
      assert.throws(() => deployment.validateArchiveListing(['-rw-r--r-- user/group 9007199254740992 2020-01-01 file']), /Release archive contains an invalid file size/);
    });

    await t.test('throws on expanded bytes limit', () => {
      const entries = Array(2).fill(`-rw-r--r-- user/group ${Math.floor(deployment.MAX_RELEASE_EXPANDED_BYTES / 1.5)} 2020-01-01 file`);
      assert.throws(() => deployment.validateArchiveListing(entries), /Release archive expands beyond the permitted size/);
    });
  });
});
