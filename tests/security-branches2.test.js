...
it('issueToken handles process.env.JWT_EXPIRY set and unset', async () => {
  const req = { identity: { userId: 'admin', authType: 'apiKey', role: 'admin', scopes: ['*'], organizationId: 'org1', teamId: 'team1' }, clientIP: '127.0.0.1' };
  ...
  await security.issueToken(req, { json: (data) => { token1 = data.token; } });
  assert.ok(token1);
  
  let token2 = null;
  delete process.env.JWT_EXPIRY;
  await security.issueToken(req, { json: (data) => { token2 = data.token; } });
  assert.ok(token2);
});

it('authenticateJWT covers stepUp and projectIds branches', async () => {
  const identity = { userId: 'admin', authType: 'oauth', role: 'admin', scopes: ['*'], projectIds: ['p1'], oauthSubject: 'sub1', oauthClient: 'c1', oauthIssuer: 'iss1' };
  const token = security.issueStepUpToken(identity);
  
  const req2 = { headers: { authorization: `Bearer ${token}` } };
  let called = false;
  let errDetails;
  await security.authenticateJWT(req2, { status: () => ({ json: (d) => { errDetails = d; } }) }, () => { called = true; });
...
it('authenticateJWT mfaVerified branch', async () => {
  ...
  const payload = { sub: 'admin', role: 'admin', scopes: ['*'], stepUp: true, mfaVerified: true, amr: ['webauthn'], authType: 'oauth', oauthSubject: 'sub1' };
  const token = jwt.sign(payload, process.env.JWT_SECRET || 's'.repeat(64), { issuer: 'mcp-server', audience: 'mcp-client', algorithm: 'HS256' });
