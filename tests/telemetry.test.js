...
describe('telemetry contract', () => {
  ...
  it('requestId handles missing, invalid, and lack of get function', () => {
    ...
    // Invalid characters
    assert.match(requestId({ get: () => 'invalid!@#' }), /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/);
    // Too long
    assert.match(requestId({ get: () => 'a'.repeat(129) }), /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/);
    // Missing value
  ...
  it('ensureFlushTimer flushes telemetry on interval', async () => {
    ...
    try {
      ...
      global.fetch = async () => { throw new Error('mock fetch failed'); };
