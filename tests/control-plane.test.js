...
describe('approval control plane', () => {
  ...
  it('runs only safe, due health-check automations', async () => {
    const automation = await controlPlane.createAutomation(
      { name: 'Hourly health', type: 'health_check', intervalMinutes: 60 },
      admin
    );
    ...
    assert.equal(
      (await controlPlane.listAutomations(admin)).find(item => item.id === automation.id).lastResult.usage.disk,
      90
    );
  });
  ...
  it('encrypts backups and rejects destinations that are not explicitly allow-listed', async () => {
    ...
    const target = await controlPlane.createBackupTarget(
      { name: 'Local vault', type: 'local', destination: process.env.BACKUP_ALLOWED_ROOTS },
      admin
    );
    ...
    await assert.rejects(
      controlPlane.registerFleetServer(
        { name: 'Untrusted', healthUrl: 'https://untrusted.example.test/health' },
        admin
      ),
      /MCP_FLEET_ALLOWED_HOSTS/
    );
  });
    ...
    const webhook = await controlPlane.createWebhook(
      {
        ...
        url: 'https://hooks.example.test/release',
        secret: 'x'.repeat(32),
