(function() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!projects.length) { list.textContent = 'No projects yet. Add a repository that is already permitted in GIT_ALLOWED_REPOS.'; return; }
      projects.forEach(project => {
        ...
        const detail = document.createElement('p'); detail.textContent = `${project.environment} · ${project.repoPath}`;
        const description = document.createElement('p'); description.textContent = project.description || 'No description provided.';
        const prompt = document.createElement('button'); prompt.className = 'btn btn-ghost'; prompt.textContent = 'Copy deployment prompt';
