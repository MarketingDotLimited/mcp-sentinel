...
4. `run_project_tests` 5. `get_project_test_run` 6. `cancel_project_test_run`
7. `read_file` 8. `write_file` 9. `delete_file`
10. `list_directory` 11. `move_file` 12. `copy_file`
13. `get_file_info` 14. `search_files`
15. `manage_service` 16. `get_service_status` 17. `list_services`
18. `get_journal_logs` 19. `manage_firewall`
20. `list_users` 21. `get_user_info` 22. `create_user`
23. `delete_user` 24. `set_user_password` 25. `modify_user`
26. `manage_ssh_keys` 27. `run_sandboxed_code`
28. `apply_config` 29. `list_config_backups` 30. `restore_config`
31. `git_operation` 32. `execute_query`
33. `list_guided_workflows` 34. `get_security_posture`
35. `request_change_approval` 36. `list_projects`
37. `get_my_ssh_access` 38. `set_my_ssh_access`
39. `list_ssh_access_policies` 40. `admin_set_ssh_access`
41. `plan_project_deployment` 42. `deploy_project`
43. `subscribe_to_alert` 44. `unsubscribe_from_alert` 45. `list_active_alerts`
